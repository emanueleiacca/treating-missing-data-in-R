...
airquality <- airquality[rowSums(is.na(airquality)) <= 3,]
...
ggplot(airquality_forward, aes(x = Day, y = Ozone)) +
  ...
  ggtitle("Ozone concentration by day and month")
```
#ancora meglio usare il KNN in questi casi. ho avuto problemi con le library per via della versione di R quindi ho dovuto usare il codice completo creato con Iodice. ho stimato il parametro di tuning via CV
```{r}
...
knn_cv_error <- function(k) {
  ...
  knn_model <- train(Ozone ~ ., data = airquality, method = "knn", trControl = trainControl(method = "cv", number = 5), tuneLength = 10, preProcess = c("center", "scale"), na.action = na.omit, tuneGrid = data.frame(k = k))
...
airquality_knn <- knn_impute(airquality, k = optimal_k)
...
ggplot(airquality_knn, aes(x = Day, y = Ozone)) +
  ...
  ggtitle("Ozone concentration by day and month")
```
#knnImputation() dal pacchetto DMwR. soluzione veloce ma comunque pesante dal punto di vista computazionale, personalmente preferisco sempre creare il codice da capo perchè lo adatti meglio ai dati essendo personalizzabile
```{r}
...
```{r}
# Carica il pacchetto "mice"
library(mice)
install.packages("mira")
# Carica il pacchetto "mira"
library(mira)

# Carica il dataset "airquality"
data(airquality)
...
airquality_imputed <- mice(airquality, m = 5, maxit = 50, method = "pmm", seed = 123)

# Esegue un'analisi di regressione lineare sui dati imputati utilizzando la funzione "with()" del pacchetto "mira"
airquality_reg <- with(airquality_imputed, lm(Ozone ~ Solar.R + Wind + Temp))
summary(airquality_reg)
```
